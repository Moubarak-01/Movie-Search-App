  ...
  return (
    <div className="relative w-full h-[48vh] min-h-[300px] sm:h-[80vh] sm:min-h-[600px] max-h-[900px] rounded-2xl overflow-hidden mt-4 md:mt-6 mb-12 shadow-2xl group">
      ...
      {items.map((item, index) => {
        ...
        const preferredMobileBg = item.poster_path 
            ? `https://image.tmdb.org/t/p/w780${item.poster_path}` 
            : item.backdrop_path ? `https://image.tmdb.org/t/p/w780${item.backdrop_path}` : '/no-movie.png';
            
        const preferredDesktopBg = item.backdrop_path 
          ? `https://image.tmdb.org/t/p/original${item.backdrop_path}`
          : item.poster_path 
            ? `https://image.tmdb.org/t/p/w1280${item.poster_path}` 
            : '/no-movie.png';

        const itemBgUrl = isMobile ? preferredMobileBg : preferredDesktopBg;
        ...
        <div className="max-w-3xl flex flex-col gap-3 animate-fade-in-up" key={currentIndex}>
          ...
          <div className="mt-4 flex gap-4">
            <button 
              ...
              className="bg-white text-black px-3 py-1.5 text-xs md:px-6 md:py-2.5 md:text-base rounded-full font-bold flex items-center gap-1.5 md:gap-2 transition-all duration-300 hover:scale-105 hover:bg-gradient-to-r hover:from-[#ab8bff] hover:to-[#8a68e3] hover:text-white hover:shadow-[0_0_20px_rgba(171,139,255,0.6)] active:scale-95 group/btn"
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5 md:w-5 md:h-5 transition-transform duration-300 group-hover/btn:rotate-12">
      ...
      <div className="absolute bottom-6 md:bottom-8 right-6 md:right-8 z-30 flex gap-2 md:gap-3">
        <button 
          onClick={handlePrev}
          className="bg-black/40 md:bg-black/20 hover:bg-black/60 md:hover:bg-black/40 backdrop-blur-md p-2 md:p-3 rounded-full text-white/80 hover:text-white transition-all hover:scale-110 active:scale-95 border border-white/10"
        >
      ...
      {/* Pagination Dots (Bottom Center) - Hidden on Mobile */}
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-30 hidden md:flex gap-2">
        {items.map((_, index) => (
          <button
            key={index}
